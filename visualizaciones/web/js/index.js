...
const getData = () => {
  $.get("http://127.0.0.1:5000/empresas/SAN.MC", function (data) {
    ...
    const dateIndex = headers.findIndex((x) => x === "Date");
    const openIndex = headers.findIndex((x) => x === "open");
    const closeIndex = headers.findIndex((x) => x === "close");
    const highIndex = headers.findIndex((x) => x === "high");
    const lowIndex = headers.findIndex((x) => x === "low");
    const volumeIndex = headers.findIndex((x) => x === "volume");
    const cciIndex = headers.findIndex((x) => x === "cci");
    ...
    dateSixMonthsAgo.setDate(dateSixMonthsAgo.getDate() - 365);

    let counter = 0;
    const indexStart = dataCandle.findIndex((x) => {
      counter++;
      return x.x >= dateSixMonthsAgo;
    });
    console.log(counter);

    const dataCandleSliced = dataCandle.slice(indexStart);
    const dataCCISliced = dataCci.slice(indexStart);
    console.log(dataCandleSliced.length);
    console.log(dateSixMonthsAgo);
